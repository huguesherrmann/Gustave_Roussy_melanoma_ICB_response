...
option_list <- list(optparse::make_option(c("-m", "--metadata") , dest="metadata_tsv"   , type="character"                                , help="Input metadata .TSV file with two columns, 'quant_sf_path' and 'sample'. See '--help' for details.")
                  , optparse::make_option(c("-t", "--threads")  , dest="num_threads"    , type="integer"     , default=1                  , help="Num processor threads to use. Default is 2.")
...
library(tibble)
library(readr)
library(dplyr) # <-- install via Bionconductor (part of the "Tidyverse")
library(Biobase)
library(edgeR) # <-- install via Bioconductor
