...
# annotation <- "/mnt/beegfs/userdata/h_herrmann/results_and_data/gene_level_analysis/annot_lookup.tsv" #annotation <- "/mnt/beegfs/userdata/h_herrmann/preprocess_RNAseq/annotation_genome_ref/gencode_hg38v24_gene_annotation_table.tsv"
...
ncg <- as.data.frame(non_coding_deseq_contrast) %>% mutate(Expression = if_else(log2FoldChange < 0, "NR", "R")) %>%
...
ncg <- get_gene_symbols(ncg, mart)
ncg <- ncg %>% arrange(desc(log2FoldChange)) %>%
  mutate(Label = if_else(row_number() <= n_top_labels & padj < 0.05, hgnc_symbol, NA))

volcano_plot <- ggplot(ncg, aes(x = log2FoldChange, y = -log10(pvalue), color = Expression, label = Label)) +
...
#
# ......................................................
ggsave(paste0(out_dir, cohort, "/DEG/coding_genes_pca.svg"), pca_plot)
ggsave(paste0(out_dir, cohort, "/DEG/coding_genes_pca.png"), pca_plot)

ggsave(paste0(out_dir, cohort, "/DEG/non_coding_volcano_plot.pdf"), volcano_plot)
ggsave(paste0(out_dir, cohort, "/DEG/non_coding_volcano_plot.svg"), volcano_plot)
ggsave(paste0(out_dir, cohort, "/DEG/non_coding_volcano_plot.png"), volcano_plot)

write.table(coding_counts, paste0(out_dir, cohort, "/DEG/coding_counts_", cohort, ".tsv"), row.names = TRUE, sep = "\t", quote = FALSE)
write.table(coding_deg, paste0(out_dir, cohort, "/DEG/coding_deg.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
write.table(non_coding_deg, paste0(out_dir, cohort, "/DEG/non_coding_deg.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
write.table(n_deg_df, paste0(out_dir, cohort, "/DEG/n_diff_expr_genes.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)
