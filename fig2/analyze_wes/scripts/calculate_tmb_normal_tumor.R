# ......................................................
# Plot TMB paired normal-tumor estimations
# 04/08/24
# Hugues HERRMANN
# ......................................................
...
maf_dir <- "/mnt/beegfs/userdata/h_herrmann/mela_ici_response_results/analyze_wes/gr1234/normal_tumor/oncotator_TvN_tsv_COSMIC/"
...
mutations_df <- data.frame(Paired_WES_tumor_sample_ID = character(),
                           N_mutations = integer())
for (maf in all_maf) {
   sample_id <- basename(maf) %>% str_split("_vs_*") %>% unlist() %>% .[1]
   
   tmp_maf <- read_tsv(maf, show_col_types = FALSE, comment = '#')
   ...
   #    filter(weak_evidence.x == "PASS")
   intersect_maf <- bed_intersect(tmp_maf %>% rename(chrom = "Chromosome", start = "Start_position", end = "End_position"), bed) %>%
      filter(Variant_Classification.x %in% non_synonymous_coding_alterations) %>%
      filter(jugement.x == "PASS")
   
   mutations_df <- mutations_df %>% add_row(data.frame(Paired_WES_tumor_sample_ID = sample_id, N_mutations = nrow(intersect_maf)))
}

mutations_df <- mutations_df %>% mutate(TMB = N_mutations / mb_targeted_genome) %>%
   inner_join(., design, by = "Paired_WES_tumor_sample_ID")
...
#
# ......................................................
write.table(mutations_df %>% select(Sample_ID, Paired_WES_tumor_sample_ID, N_mutations, TMB), paste0(out_dir, "normal_tumor/tmb/tmb.tsv"), sep = "\t", quote = FALSE, row.names = FALSE)

ggsave(paste0(out_dir, "normal_tumor/tmb/tmb.pdf"), tmb_plot, width = 3, height = 3.5)
ggsave(paste0(out_dir, "normal_tumor/tmb/tmb.png"), tmb_plot, width = 3, height = 3.5)
